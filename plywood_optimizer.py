...
# --- The core packing functions (_pack_sheet, _solve_for_type) are unchanged ---
# You can copy them from the previous version. The only change is in create_cut_list.
...
def create_cut_list(plywood_sheets, required_pieces, saw_kerf):
    """
    Orchestrates the optimization. Now includes a pre-processing step to detect
    and solve for identical subsets of pieces.
    """
    STRATEGIES = [
    ...
    for plywood_type, pieces in pieces_by_type.items():
        ...
        if divisor > 1:
            ...
            # Create a single kit of pieces
            single_kit = []
            for fingerprint, count in counts.items():
                for _ in range(count // divisor):
                    # Find an original piece that matches the fingerprint to preserve name etc.
            ...
            # Give the kit unique IDs before optimizing
            for i, p in enumerate(single_kit):
                p['id'] = p.get('name', 'Piece') + f"_kit1_{i+1}"

            # Run the contest on just the single, smaller kit
            kit_layout = _run_strategy_contest(sheet_dims, single_kit, saw_kerf, STRATEGIES)
            
            # Multiply the result
            final_layout[plywood_type] = kit_layout * divisor
            continue # Skip the full optimization for this type

        # --- Fallback to standard optimization if no subsets are found ---
...
def _run_strategy_contest(sheet_dims, pieces, saw_kerf, strategies):
    """Helper function to run the optimization contest for a given set of pieces."""
    best_layout = None
    ...
    if total_sheet_area > 0:
        ...
        print(f"     Efficiency: {waste_percentage:.2f}% of material is waste.")
